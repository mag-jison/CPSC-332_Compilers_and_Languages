...
public:
   ...
   void handler(const size_t&, const string&, const string&); // handles syntatical errors; errors due to grammar and structure of the program; the incorrect ordering of a sentence
...
void ErrorTable::handler(const size_t& ln, const string& p, const string& q){
      error.push("\033[91m");
      if (q == "<E>")
         error.push("INVALID EXPRESSION"); // error types
      else if (q == "<ID>")
         error.push("INVALID IDENTIFIER");
      else if (q == "= | ( | <ID> | <NUM>" || q == "end" || q == "do" || q == "whileend" || q == "then" || q == "else" || q == "endif")
