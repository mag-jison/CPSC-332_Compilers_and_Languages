    ...
    if (on){
        switch (n){
            ...
            case TYPE:
                ...
                break;
            case IDENTIFIER:
                insert(table.front(), CURR_TYPE, CURR_SCOPE);
...
bool PRDP::F(){
    ...
    if (lexeme == "("){
        ...
        SAVE_TYPE = inType("IDENTIFIER", SAVE_TYPE, token, lexeme, ln);
        lexer();
        return true;
    }
    else if (FIRST(NUMBER)){
        printRules(NUMBER);
        SAVE_TYPE = inType("NUMBER", SAVE_TYPE, token, lexeme, ln);
        lexer();
        return true;
    }
    else {
