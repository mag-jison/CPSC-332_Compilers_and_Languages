   ...
   if (empty()){ 
      while (!q.empty()){
         switch (q.front().state){ 
            case 1:
               isK(s) ? insert(q.front().ln, "KEYWORD", q.front().lexeme) : insert(q.front().ln, "IDENTIFIER", q.front().lexeme);
               break;
            case 2:
               insert(q.front().ln, "INTEGER", q.front().lexeme);
               break;
            case 3:
               insert(q.front().ln, "REAL", q.front().lexeme);
               break;
            case 4:
               insert(q.front().ln, "SEPARATOR", q.front().lexeme);
               break;
            case 5:
               insert(q.front().ln, "OPERATOR", q.front().lexeme);
               break;
            case 6: 
               insert(q.front().ln, "PUNCTUATION", q.front().lexeme);
               break;
            default:
               break;
         }
         ...
         switch (q.front().state){ 
            case 1:
               if (isK(q.front().lexeme)){
                  ...
                  insert(q.front().ln, "KEYWORD", q.front().lexeme);
               }
               else {
                  cout << right << "| IDENTIFIER" << setw(5) << "=" << right << setw(13) << q.front().lexeme << " |" << endl;
                  insert(q.front().ln, "IDENTIFIER", q.front().lexeme); 
               }
               break;
            case 2:
